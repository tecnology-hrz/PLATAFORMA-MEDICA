...
document.getElementById('citaForm').addEventListener('submit', async (e) => {
    ...
    showLoadingModal('Agendando cita, enviando correos y creando evento en calendario...');
    ...
    try {
        ...
        try {
            if (window.GoogleCalendar && window.GoogleCalendar.isReady() && window.GoogleCalendar.isAuthorized()) {
                ...
                const eventDetails = {
                    summary: `Cita: ${pacienteData.nombre} - ${getTipoCitaText(citaData.tipoCita)}`,
                    description: `Paciente: ${pacienteData.nombre}\nCédula: ${pacienteData.cedula}\nTipo: ${getTipoCitaText(citaData.tipoCita)}\nMotivo: ${citaData.motivoCita}\nMédico: ${citaData.medicoNombre}`,
                    startDateTime: startDateTime,
                    endDateTime: endDateTime,
                    attendees: [
                        { email: pacienteData.email },
                        ...(medicoData && medicoData.email ? [{ email: medicoData.email }] : [])
                    ]
                };
